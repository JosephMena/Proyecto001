var app = angular.module('myApp', ['ngMaterial', 'ngMessages',
            'angularSpinner', 'ui.grid', 'ui.grid.resizeColumns',
            'ui.grid.moveColumns']);
...
app.controller('MainCtrl', function ($scope, $rootScope, $http, $log,
            $mdDialog, $mdSidenav, $timeout, $filter, ajaxService,
            usSpinnerService) {
  ...
  $scope.showAbout = function(ev) {
    ...
    $mdDialog.show(
      $mdDialog.alert()
        ...
        .textContent('This application will show you many of the features available in Angular Material, Spring Framework and MongoDB...')
        .ariaLabel('Employee Directory Demo')
        .ok('Ok').openFrom('#about').closeTo('#about').targetEvent(ev));
    };
  ...
  $scope.standardSearch = function (firstName, lastName) {
    ...
    function onSuccess(response) {
      ...
      if ($scope.debugFlag === 'true' || $scope.debugFlag) {
        console.log("Inside standardSearch response..."
    ...
    var addl_params ='?firstName=' + firstName + '&lastName='
                        + lastName + '&etc=' + new Date().getTime();
 
    //----MAKE AJAX REQUEST CALL to POST DATA----
    ajaxService.postData(standardSearchUrl, 'POST', null,
        addl_params).then(onSuccess, onError);
  ...
  $scope.advancedSearch = function (criteriaSearch) {
    ...
    console.log("multipleCriteria JSON...: "
      + JSON.stringify(criteriaSearch));
    $scope.gridActive = true;
 
    function onSuccess(response) {
      console.log("+++++advancedSearch SUCCESS++++++");
      if ($scope.debugFlag === 'true' || $scope.debugFlag) {
        console.log("Inside advancedSearch response..."
          + JSON.stringify(response.data));
      } else {
    ...
    //----MAKE AJAX REQUEST CALL to POST DATA----
    ajaxService.postData(advancedSearchUrl, 'POST',
      criteriaSearch, '').then(onSuccess, onError);
  };
 
  $scope.setDefaults = function(debugFlag, baseUrl) {
    standardSearchUrl = baseUrl + "/EmployeeDirectory/rest/standardSearch";
    advancedSearchUrl = baseUrl + "/EmployeeDirectory/rest/advancedSearch";
